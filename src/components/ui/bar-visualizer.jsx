...
const BarVisualizerComponent = React.forwardRef((
  ...
  const highlightedIndices = useBarAnimator(state, barCount, state === "connecting"
    ? 2000 / barCount
    : state === "thinking"
      ? 150
      : state === "listening"
...
const BarVisualizer = React.memo(BarVisualizerComponent, (prevProps, nextProps) => {
  ...
  prevProps.className === nextProps.className && JSON.stringify(prevProps.style) === JSON.stringify(nextProps.style));
})
